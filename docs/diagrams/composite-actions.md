...
This repository uses composite actions from [elioetibr/composite-actions@v3](https://github.com/elioetibr/composite-actions) that encapsulate common CI/CD patterns used across all build workflows. These actions follow the DRY principle and provide a single source of truth for shared functionality.

> **Note:** The composite actions have been migrated from `.github/actions/` in this repository to the centralized `elioetibr/composite-actions` repository and are now available at version `v3`.
...
flowchart TB
    ...
    subgraph actions["Composite Actions (elioetibr/composite-actions@v3)"]
        CA1["aws-ecr-setup"]
...
The composite actions are now hosted in [elioetibr/composite-actions@v3](https://github.com/elioetibr/composite-actions):

- `elioetibr/composite-actions/aws-ecr-setup@v0` - AWS OIDC + ECR login
- `elioetibr/composite-actions/docker-buildx-setup@v0` - Docker BuildX configuration
- `elioetibr/composite-actions/github-auth-checkout@v0` - GitHub App auth + checkout
- `elioetibr/composite-actions/gitversion-calculate@v0` - GitVersion with caching
